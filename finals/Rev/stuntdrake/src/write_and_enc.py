...
if __name__ == "__main__":
    ...
    for i in range(60, 62):
        convert_bin_to_hex(f"golden_titus_{i-59}", f"{i-59}:")
    for i in range(62, 64):
        convert_bin_to_hex(f"twilight_broodmother_{i-61}", f"{i-61}:")
    for i in range(64, 66):
        convert_bin_to_hex(f"golden_twilight_hatchling_{i-63}", f"{i-63}:")
    convert_bin_to_hex("stuntdrake", "ST:")
    key = b"FF"
